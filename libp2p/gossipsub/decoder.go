...
func (d Decoder) New() instrument.StreamDecoder {
	max := d.MaxRPCSize
	if max == 0 {
...
func (i *instance) ObserveRead(data []byte, emit instrument.EmitFunc) error {
	return i.in.observe(i.msgDecoder, data, emit)
}

func (i *instance) ObserveWrite(data []byte, emit instrument.EmitFunc) error {
...
func (v *bufferingRPC) observe(msgDecoder MessageDecoder, data []byte, emit instrument.EmitFunc) error {
	...
	for {
		...
		if framing := totalLen - innerBytes; framing > 0 {
			emit(framing, []instrument.Tag{{Name: TagFraming}}, nil)
		}
...
func emitActions(rpc *pb.RPC, msgDecoder MessageDecoder, emit instrument.EmitFunc) int {
	inner := 0
	...
	if ctrl := rpc.Control; ctrl != nil {
		...
		for _, iwant := range ctrl.Iwant {
			size := iwant.Size()
			inner += size
			emit(size, []instrument.Tag{{Name: TagMessageKind, Values: []string{"IWANT"}}}, iwant)
		}
		...
		for _, idw := range ctrl.Idontwant {
			size := idw.Size()
			inner += size
			emit(size, []instrument.Tag{{Name: TagMessageKind, Values: []string{"IDONTWANT"}}}, idw)
		}
	}
...
func emitTopicControl(size int, kind string, topicID *string, parsed any, emit instrument.EmitFunc) int {
	tags := make([]instrument.Tag, 1, 2)
	tags[0] = instrument.Tag{Name: TagMessageKind, Values: []string{kind}}
	if topicID != nil {
		tags = append(tags, instrument.Tag{Name: TagTopic, Values: []string{*topicID}})
	}
	emit(size, tags, parsed)
	return size
}

func tagsFromMap(m map[string][]string) []instrument.Tag {
	tags := make([]instrument.Tag, 0, len(m))
	for name, values := range m {
		tags = append(tags, instrument.Tag{Name: name, Values: values})
	}
	return tags
}
