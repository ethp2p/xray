...
import (
	...
	"github.com/ethp2p/instrument/libp2p/gossipsub"
...
func TestGossipSub_TopicTagging(t *testing.T) {
	...
	for _, sink := range []*testSink{sink1, sink2} {
		for _, evt := range sink.Events() {
			if bw := evt.GetTraffic(); bw != nil {
				totalBytes += uint64(bw.Bytes)
			}
		}
	}
	if totalBytes == 0 {
...
func TestGossipSub_DecoderFallback(t *testing.T) {
	...
	sink1 := newTestSink()
	tagCollector := newTagCollector()
	host1, err := instrument.Wrap(baseHost1,
		instrument.WithDecoder(gossipsub.Decoder{}.Match, gossipsub.Decoder{}.New),
		instrument.WithOnMessage(tagCollector.Factory()),
		instrument.WithSink(sink1),
	)
	if err != nil {
	...
	sink2 := newTestSink()
	host2, err := instrument.Wrap(baseHost2,
		instrument.WithDecoder(gossipsub.Decoder{}.Match, gossipsub.Decoder{}.New),
		instrument.WithOnMessage(tagCollector.Factory()),
		instrument.WithSink(sink2),
	)
	if err != nil {
	...
	events := sink1.Events()
	for _, evt := range events {
		if bw := evt.GetTraffic(); bw != nil && bw.Bytes > 0 {
			foundTrafficEvent = true
			if int(bw.Bytes) >= n {
				t.Logf("found traffic event with %d bytes (wrote %d)", bw.Bytes, n)
			}
		}
	}
...
func (tc *tagCollector) Factory() instrument.OnMessageFactory {
	return func(streamID uint32, protocol string) instrument.OnMessage {
		return func(msg instrument.DecodedMessage) {
			tc.mu.Lock()
			defer tc.mu.Unlock()
...
func (s *testSink) Write(event *pb.TraceEvent) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *testSink) Events() []*pb.TraceEvent {
	s.mu.Lock()
	defer s.mu.Unlock()
	result := make([]*pb.TraceEvent, len(s.events))
...
func setupTwoHostsWithPubsub(t *testing.T, ctx context.Context, tc *tagCollector) (*instrument.Host, *instrument.Host, *testSink, *testSink) {
	baseHost1, err := libp2p.New(libp2p.ResourceManager(&network.NullResourceManager{}))
	if err != nil {
		t.Fatalf("failed to create base host1: %v", err)
	}
	...
	host1, err := instrument.Wrap(baseHost1,
		instrument.WithDecoder(gossipsub.Decoder{}.Match, gossipsub.Decoder{}.New),
		instrument.WithOnMessage(tc.Factory()),
		instrument.WithSink(sink1),
	)
	if err != nil {
		baseHost1.Close()
